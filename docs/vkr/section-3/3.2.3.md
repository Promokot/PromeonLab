...
> Статус: ЧЕРНОВИК Клода (2026-06-05) по утверждённому плану plan-3.2.3.md. Структура блока после Рисунка 3.9 финализирована интерактивно с Максом: граф (представление и умения) → нода с паспортом (Листинг 3.29 SceneNode) → запуск и обход загрузки (Листинг 3.30 SceneGraph) → обозреватель (3.31) → CaptureSnapshot → финал. Решения: тонкость запуска в абзаце загрузки; мысль про метку-признак выкинута; отсылка «в пункте 3.2.4»; ручные правки Макса сохранены.
