...
> Статус: ЧЕРНОВИК Клода (2026-06-05) по утверждённому плану plan-3.2.3.md (решения Макса: SceneListNode_Item и SceneNode целиком в тексте, CaptureSnapshot словами, меню и пикер одним кадром, поток открытия без схемы). Правки 2026-06-05: обратные отсылки прорежены до двух (переход 3.2.1, автосохранение 3.1.4); рисунки сдвинуты на 3.9/3.10; блок после Рисунка 3.9 пересобран по варианту A-бридж (загрузка → нода → граф → обозреватель → снимок), цепочка перехода сжата до ссылки на 3.2.1; ручные правки Макса сохранены.
...
Упомянутый компонент SceneNode – паспорт объекта сцены (см. Листинг 3.30). Он хранит идентификатор ноды, ссылку на ассет AssetRef, отображаемое имя и флаги видимости и блокировки. Методы установки синхронизируют имя и видимость с игровым объектом, а по наличию компонента ноды сцены отличаются от вспомогательных объектов.
