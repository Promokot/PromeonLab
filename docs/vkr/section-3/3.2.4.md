...
> Статус: ЧЕРНОВИК v1 (2026-06-05), на ревью Максу. План и решения: plan-3.2.4.md.
...
Наполнение импортированной библиотеки выполняет конвейер импорта ImportPipeline – объект корневой области жизни, подписанный на сообщения выбора файла и подтверждения импорта. Общий ход конвейера показан на Рисунке 3.11. Начинается он с кнопки добавления в браузере ассетов, открывающей файловый браузер, выбранный в диалоге путь приходит конвейеру сообщением FilePickedEvent. Поддерживаемые форматы описаны набором импортёров – классов интерфейса IAssetImporter, зарегистрированных списком: GltfAssetImporter принимает модели форматов .glb и .gltf, ImageAssetImporter – изображения .png, .jpg и .jpeg. Конвейер подбирает импортёр по расширению выбранного файла (см. Листинг 3.33), а для файла неподдерживаемого формата ограничивается предупреждением в журнале. Найденный импортёр определяет предлагаемый тип ассета – Object для моделей и Reference для изображений, и конвейер публикует запрос ImportRequestedEvent с путём, именем файла и этим типом. Полный текст конвейера приведён в Приложении Б (Листинг Б.14).

[РИСУНОК 3.11 – заготовка (СХЕМА – генерирует Клод): конвейер импорта одной цепочкой: файловый браузер → FilePickedEvent → ImportPipeline (выбор импортёра по расширению) → ImportRequestedEvent → мастер импорта → ImportConfirmedEvent → копирование источника в sources → построение шаблона восстановления → миниатюра в thumbnails → запись в imported-lib.json → AssetImportedEvent → обновление браузера]
*Рисунок 3.11 – Конвейер импорта ассетов*

Листинг 3.33 – Скрипт ImportPipeline.cs (Фрагмент выбора импортёра)
```csharp
...
Запрос открывает мастер импорта ImportWizardPanel (см. Рисунок 3.12). Мастер показывает имя выбранного файла, подставляет предлагаемое имя ассета в поле ввода и выставляет предлагаемый тип. Пользователь может изменить имя и переключить тип на любой из трёх – так модель со скелетом переводится в тип Rig, поскольку автоматическое распознавание скелета потребовало бы полной загрузки модели ещё до подтверждения импорта. Для скелетных моделей предусмотрена группа переключателей оси концевых костей: у крайних костей скелета нет дочерней кости, по которой выводится направление, поэтому ось задаётся явно с необязательной инверсией, а без выбора определяется автоматически по направлению родительской кости. Кнопка импорта публикует сообщение ImportConfirmedEvent с введёнными значениями, отмена – то же сообщение с признаком Confirmed, равным false, по которому конвейер ничего не предпринимает. Полный текст мастера приведён в Приложении Б (Листинг Б.15).
...
Подтверждённый импорт конвейер выполняет асинхронно (см. Листинг 3.34). Импортёр создаёт запись: ассету назначается короткий случайный идентификатор, копия исходного файла помещается в каталог sources хранилища, и в запись попадает относительная ссылка на неё. Затем строится шаблон восстановления: реестр билдеров загружает скопированный источник и фиксирует решения о коллайдере, слое взаимодействия и параметрах размещения, а для скелетных моделей в шаблон дополнительно вносится выбранная в мастере ось концевых костей. Чтение файлов glTF во всех случаях выполняет загрузчик GltfModelImporter, построенный на сторонней библиотеке glTFast. Запись с готовым шаблоном и миниатюрой добавляется в импортированную библиотеку, библиотека сохраняется на диск, и сообщение AssetImportedEvent извещает интерфейс о новом ассете. Ошибка любого шага записывается в журнал и прерывает только текущий импорт.

Листинг 3.34 – Скрипт ImportPipeline.cs (Фрагмент выполнения импорта)
```csharp
...
Галереей библиотек служит браузер ассетов AssetBrowserPanel – панель-модуль регионной модели (см. Рисунок 3.13). Три вкладки переключают активную библиотеку, по записям которой строится сетка карточек – элементов списка LabAsset_Item. Карточка показывает миниатюру с именем и извещает панель о выборе. Миниатюру карточке выдаёт метод ResolveIcon (см. Листинг 3.36): у встроенного ассета берётся спрайт записи, у импортированного изображение загружается из файла по ссылке ThumbnailRef и кешируется, причём кешируется и неудачная попытка, чтобы ошибочная ссылка не перечитывалась при каждом перестроении сетки. Кнопка размещения активна при выбранной карточке в режимах редактирования и песочницы. Кнопка удаления доступна вне встроенной библиотеки и стирает запись вместе с файлом-источником. Кнопка добавления открывает файловый браузер, который делит регион с браузером ассетов и мастером, поэтому на время выбора файла панель скрывается и возвращает себя, когда регион пустеет. Полный текст панели приведён в Приложении Б (Листинг Б.17).

[РИСУНОК 3.13 – заготовка (СКРИНШОТ, готовит Макс): браузер ассетов, вкладка импортированной библиотеки с несколькими миниатюрами, выбранная карточка, активные кнопки размещения и удаления]
*Рисунок 3.13 – Браузер ассетов с миниатюрами*

Листинг 3.36 – Скрипт AssetBrowserPanel.cs (Фрагмент загрузки миниатюр)
```csharp
...
Размещение начинается в браузере: кнопка вычисляет точку на полу в 1,2 метра перед камерой и разворот, обращающий лицевую сторону объекта к пользователю, после чего публикует сообщение AssetSpawnRequestedEvent с выбранной записью. Принимает его AssetSpawner – объект внутрисценной области жизни (см. Листинг 3.37). К запрошенной позиции прибавляется смещение из шаблона – так изображения-референсы при первом размещении поднимаются над полом. При загрузке сцены смещение повторно не применяется, поскольку сохранённая позиция уже содержит его. Затем объект восстанавливается реестром билдеров, регистрируется нодой в графе сцены и проходит внедрение зависимостей по всей иерархии.

Листинг 3.37 – Скрипт AssetSpawner.cs
```csharp
...
Шаблоны встроенных ассетов готовятся заранее, на этапе разработки. Инструмент редактора Unity BuiltinRecipeBaker записывает готовый шаблон в каждую запись встроенной библиотеки (см. Листинг 3.39): префаб записи загружается в изолированную превью-сцену, не затрагивая открытую сцену разработчика, и измеряется тем же синхронным ядром, которым пользуются билдеры при импорте, поэтому встроенный и импортированный ассеты одного типа получают одинаковые шаблоны. Для записей-референсов по исходному изображению дополнительно генерируется и сам префаб. Результат вносится в приватные сериализуемые поля записи через рефлексию, и типы времени выполнения остаются свободными от кода редактора. В сборку приложения инструмент не входит. Полный текст приведён в Приложении Б (Листинг Б.19).

Листинг 3.39 – Скрипт BuiltinRecipeBaker.cs (Фрагмент подготовки записи библиотеки)
```csharp
