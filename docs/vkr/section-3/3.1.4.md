...
Запись данных на диск не привязана к ручной команде сохранения: приложение само определяет моменты, когда состояние должно попасть в хранилище. При этом два файла сцены живут в разном ритме. Файл scene.json записывается дважды за сеанс – при создании сцены и при завершении редактирования, тогда как animation.json обновляется непрерывно, по мере правки ключевых кадров. Различие объясняется характером данных: граф сцены меняется сравнительно редко и его выгодно фиксировать целиком, а анимация правится длинными сериями мелких операций, потеря которых обошлась бы дороже всего.
