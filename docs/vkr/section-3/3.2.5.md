...
Каждый прокси-объект собирается по единому составу (см. Листинг 3.44). Он получает меш-ромб с материалом и обводкой стороннего пакета QuickOutline, выпуклый меш-коллайдер по форме ромба, компоненты ноды SceneNode с маркером прокси-кости BoneSceneNodeMarker, признак выбираемости и компонент взаимодействия на отдельном слое костей. На настоящую кость модели навешивается компонент следования BoneFollower со ссылкой на построенный прокси. Дочерние прокси строятся рекурсивно внутри родительского, повторяя иерархию скелета.
