...
> Статус: ПРИНЯТ Максом (2026-06-04, v3); v4 (2026-06-04) – по решению Макса добавлен абзац о сценных областях с Листингами 3.13–3.14, последующие листинги перенумерованы (3.15–3.19). Дополнение НА ПРОВЕРКЕ. 2026-06-04 – стилистическая чистка по правилу 5.12. 2026-06-04 (ревью Макса) – термин «сервис» заменён на «объект»/«класс», каркас переформулирован как условная группировка механизмов (решения 12–14 plan-3.2.md), правки Макса учтены.
...
Время жизни объектов организовано через иерархию областей жизни (LifetimeScope). Каждая область конфигурирует собственный контейнер зависимостей – реестр, который создаёт объекты и передаёт их друг другу через параметры конструкторов. Подход реализован библиотекой VContainer – легковесной и широко распространённой в Unity-проектах. Она снимает ручную сборку объектов и поддерживает вложенные области без дополнительного кода [ИСТ: VContainer]. Корневая область RootLifetimeScope создаётся при запуске, переживает любые смены сцен и содержит объекты с временем жизни приложения. К ним относятся PathProvider, AppStorage и EventBus, а также библиотеки ассетов и конвейеры импорта и экспорта. Сценная область создаётся вместе со сценой режима и уничтожается при выходе из него; контейнер сценной области вложен в корневой, поэтому сценный объект может зависеть от корневого, но не наоборот. Правило направления зависимостей иллюстрирует пример из пункта 3.1.4: сценный SceneAutoSaver получает через конструктор корневые AppStorage и EventBus, тогда как обратная связь – корневой объект, удерживающий сценный, – исключена самой иерархией. Одновременно разрешено существование только одной сценной области. Состав областей приведён в Таблице 3.1.
...
В листинге применяются два способа регистрации, и различие между ними определяется ролью объекта. Большинство объектов пассивны: такой объект создаётся контейнером в момент, когда понадобится другому, и работает только в ответ на обращения – для этого достаточно обычного Register. Однако часть объектов должна действовать самостоятельно. Так, библиотека импортированных ассетов ImportedAssetLibrary из того же листинга обязана прочитать свой файл с диска сразу при запуске приложения, без обращения со стороны других объектов. Компонентам Unity подобные возможности предоставляет движок, вызывая их методы Start и Update, но многие зарегистрированные в контейнере классы не наследуют MonoBehaviour – их экземпляры движок не отслеживает и методов жизненного цикла у них не вызывает. Для таких классов предусмотрена регистрация RegisterEntryPoint: жизненный цикл берёт на себя контейнер и сам вызывает объект в нужные моменты. О том, какие именно моменты ему нужны, класс заявляет реализацией интерфейса: IStartable означает вызов при построении области, ITickable – вызов каждый кадр. По этой же схеме работает, например, SceneDirtyTracker: контейнер сценной области вызывает его метод Start, в котором объект оформляет подписки на сообщения. Покадровые вызовы применяются при воспроизведении анимации.

Из-за того, что наборы объектов в сценных областях различаются, объектам корневой области нельзя удерживать прямые ссылки на сценные: после смены режима такая ссылка указывала бы на уничтоженный объект. Посредником служит корневой объект SceneContext – фасад, через который постоянные потребители, прежде всего панели интерфейса, обращаются к объектам текущей сцены. Полный текст фасада приведён в Листинге 3.15.
...
Заполнением и очисткой фасада занимается объект сценной области SceneContextBinder, приведённый в Листинге 3.16. При старте сценной области он заполняет фасад ссылками на граф сцены, выделение и анимационные объекты и публикует сообщение SceneContextChangedEvent; при уничтожении области фасад очищается с повторной публикацией сообщения. Заполнение выполняется защищённо: метод Resolve перехватывает отказ контейнера и оставляет в фасаде пустую ссылку, если класс в текущей области не зарегистрирован. Так, в песочнице анимационные классы не зарегистрированы, поэтому потребитель проверяет доступность именно того объекта, который собирается использовать, а не факт загруженной сцены.
...
Регистрация в сценных областях имеет особенность: заметная часть их объектов существует не в виде чистых классов, а в виде компонентов, заранее размещённых в сцене режима. Область главного меню компактна (Листинг 3.13): помимо отслеживания изменений в ней регистрируются две панели, и для них применяется третий способ регистрации – RegisterComponentInHierarchy, при котором контейнер не создаёт объект, а находит готовый компонент в иерархии загруженной сцены и берёт его на учёт.
...
[РИСУНОК 3.4 – заготовка (СХЕМА, генерирует Клод): корневая область с перечнем объектов наверху; ниже три сценные области (MainMenu / VrEditing / Sandbox) с их наборами; стрелки вложенности контейнеров снизу вверх; сбоку SceneContext-фасад с подписью «доступ постоянных потребителей к сценным объектам»]
