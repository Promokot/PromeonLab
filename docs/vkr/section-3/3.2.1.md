...
Заполнением и очисткой фасада занимается объект внутрисценной области SceneContextBinder, приведённый в Листинге 3.16. При старте внутрисценной области он заполняет фасад ссылками на граф сцены, выделение и анимационные объекты и публикует сообщение SceneContextChangedEvent; при уничтожении области фасад очищается с повторной публикацией сообщения. Заполнение выполняется защищённо: метод Resolve перехватывает отказ контейнера и оставляет в фасаде пустую ссылку, если класс в текущей области не зарегистрирован. Так, в песочнице анимационные классы не зарегистрированы, поэтому потребитель проверяет доступность именно того объекта, который собирается использовать, а не факт загруженной сцены.
...
Третий механизм каркаса – переключение режимов. Каждому режиму соответствует собственная сцена Unity и внутрисценная область; с режимом связаны и доступные пользователю возможности: состав панелей интерфейса и кнопок навигации определяется режимом и перестраивается при его смене – подробно это поведение разбирается при описании системы интерфейсов. Сменой режимов управляет класс ModeOrchestrator: он проверяет допустимость перехода по графу ModeTransitionGraph и делегирует фактическую перезагрузку сцены исполнителю переходов. Граф задан как ассет с перечнем разрешённых пар: меню соединено двусторонними переходами с редактированием и песочницей, прямой переход между редактированием и песочницей не предусмотрен (Рисунок 3.5). Полный текст оркестратора приведён в Листинге 3.17.
