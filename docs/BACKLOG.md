...
| Feature | Status | Notes | Ref |
|---|---|---|---|
| **Detachable / floating panels** (`SpatialPanelDetachable`, `DetachablePanelDragHandle`, `PanelDetachAddon`) | STUB (neutralized) | Feature wired into **no live UX** — panels are governed by the root-lifetime region router (`PanelRegionRouter`); the planned dock-vs-detach add-on split (`PanelDetachAddon`) was never built. The lingering `SpatialPanelDetachable` (on `AnimatorPanelModule.prefab`) had its **operational code commented out (inert) 2026-06-02** because its `Awake()` `SetActive(false)` ate the first region-router Open (two-press-to-open bug). Class shell + serialized fields kept so prefab refs resolve and the feature can be revived by uncommenting. `DetachablePanelDragHandle` is on no prefab and now calls an inert `MoveDelta`. | audit 05 §4 |
